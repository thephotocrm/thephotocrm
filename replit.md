...
*   **Selection Locking:** After client signs contract, all package and add-on selections are permanently locked to preserve contract integrity. UI shows disabled state with warning message, and backend validates to prevent tampering (returns 400 error if selections change after signature).
