...
*   **AI Automation Builder:** Natural language automation creation using OpenAI GPT-4o-mini with structured outputs and two-step confirmation flow. Photographers describe automations in plain language (e.g., "Send welcome email when someone enters inquiry stage"), and AI extracts structured parameters (trigger type, stages, delays, content) via Zod schema validation. **Two-Step Flow:** (1) AI extracts parameters via `/api/automations/extract-with-ai` and shows confirmation dialog with automation details and stage selection dropdown; (2) User reviews/adjusts and confirms via `/api/automations/create-with-ai` to finalize creation. Features smart interpretation of timing (converts "1 day later" to delayDays: 1), automatic content generation, multi-step sequences, and stage-based trigger selection. **Personalized Placeholders:** Supports `{{PHOTOGRAPHER_NAME}}`, `{{BUSINESS_NAME}}`, and `{{SCHEDULING_LINK}}` placeholders that are automatically replaced with photographer-specific values from settings. **Smart File Support:** AI recognizes requests to send proposals, invoices, or contracts and creates SMART_FILE type automations. Confirmation dialog shows template dropdowns for ALL Smart File steps (not just first), allowing photographers to select exact templates from their library rather than AI guessing. Multi-step support ensures each SMART_FILE step gets proper template selection with validation. **Message Generation:** AI generates ready-to-use messages without non-functional placeholders (no `[Your Name]`, `{Business Name}`), but intelligently includes `{{SCHEDULING_LINK}}` placeholder when photographer requests to send booking/scheduling links. **Editable Content:** Confirmation dialog displays AI-generated message in an editable textarea, allowing photographers to modify content before creation. Uses GPT-4o-mini with JSON response format for reliable parameter extraction. Accessible at Marketing → Automations with purple sparkle icon. **Default Stages:** New photographers automatically receive 5 default WEDDING stages: Inquiry (default), Consultation, Payment Made, Booked, Finished.
