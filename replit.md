...
*   **Magic Link Portal System:** Passwordless client authentication for one-click portal access via secure, time-limited email/SMS links. Features include: (1) Client Portal Login Links - photographers can send magic links from contact detail pages; (2) Rate limiting (max 100 tokens per hour per contact); (3) Auto-login via GET /api/client-portal/validate/:token endpoint that validates tokens, creates user accounts if needed, and issues httpOnly JWT cookies; (4) Client-portal page handles ?token= query parameter with loading/error states; (5) Password login remains available as backup authentication method; (6) 7-day token expiration with automatic validation in storage layer.
