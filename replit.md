...
*   **Automatic DNS Provisioning:** Cloudflare API integration that automatically creates, updates, and deletes DNS records when photographers manage their portal slugs. When a photographer sets their slug in Settings, the system creates a CNAME record (`{slug}.tpcportal.co` → `tpcportal.co`) via Cloudflare API with DNS-only (gray cloud) status for Replit SSL compatibility. Supports slug changes (deletes old record, creates new), slug removal (deletes record), and includes error resilience that logs DNS failures without blocking photographer updates. Uses structured logging for operational visibility and skips DNS operations when portalSlug is not in update payload to prevent accidental deletions during partial updates.
