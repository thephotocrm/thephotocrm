...
*   **AI Automation Builder:** Natural language automation creation using OpenAI GPT-5 with structured outputs. Photographers describe automations in plain language (e.g., "Send welcome email when someone enters inquiry stage"), and AI extracts structured parameters (trigger type, stages, delays, content) via Zod schema validation. Features "Create with AI" button on automations page, smart interpretation of timing (converts "1 day later" to delayDays: 1), automatic content generation, and support for multi-step sequences. Uses GPT-4o-mini with JSON response format for reliable parameter extraction. Accessible at Marketing → Automations with purple sparkle icon.
