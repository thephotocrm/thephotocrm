...
*   **AI Chatbot for Client Support:** Intelligent chatbot assistant powered by OpenAI GPT-5 to help clients with questions. Appears as a floating widget on both authenticated photographer pages and public client-facing pages (Smart Files, booking calendars). Features include context-aware responses based on page location, conversation history, personalized greetings with photographer's business name, and comprehensive knowledge of CRM features. Public API endpoint requires no authentication, enabling seamless client support.
