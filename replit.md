...
*   **Core Features:** Template system, drag-and-drop builder, 6 page types (Text, Package Selection, Add-ons, Contract, Payment, Form), project integration, public responsive client view, status tracking (DRAFT → SENT → VIEWED → ACCEPTED → PAID), integrated Stripe Connect checkout with platform fee, flexible payment options (online/offline, zero deposit), selection persistence, and multiple package selection support.
*   **Form Page Type:** Custom form builder for client intake and questionnaires with section/block architecture (same as TEXT pages). Supports 1-column and 2-column layouts with 7 field types: TEXT_INPUT, TEXTAREA, MULTIPLE_CHOICE (radio), CHECKBOX (multi-select), NUMBER, DATE, EMAIL. Each field configurable with label, placeholder, required flag, and options for choice fields. Forms rendered with proper validation, onChange handlers, and submission to backend. Answers stored in projectSmartFiles.formAnswers as JSON object mapping block IDs to values (arrays for checkboxes, strings for others). Form responses displayed in project detail page with question-answer pairs extracted from pagesSnapshot. Logs FORM_SUBMITTED activity to project history.
