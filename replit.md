...
*   **Smart Files Routing Fix (Nov 2024):** Fixed 404 error when clicking on Smart Files from client portal. Changed incorrect route from `/estimates/{token}` to correct `/smart-file/{token}` in both Files tab (line 491) and Payments tab (line 589) of client-portal-project.tsx. Ensures Smart Files now properly load from client portal views instead of returning 404 errors.
*   **Client Portal Avatar Initials Fix (Nov 18, 2024):** Fixed missing avatar initials for CLIENT users in client portal sidebar. Root cause was CLIENT users lacking `clientId` field linking to contact records. Implemented comprehensive fix: (1) Updated `/api/auth/me` to fetch firstName/lastName from contacts table via clientId for CLIENT role, (2) Modified CLIENT user creation in 3 locations (magic link login, portal token validation, Google OAuth) to populate clientId field, (3) Added backfillClientIds() storage method for safe retrospective updates, (4) Created admin-only `/api/admin/backfill-client-ids` endpoint with dry-run mode and duplicate detection for safely updating production accounts. TypeScript User interface extended with optional firstName/lastName fields. Backfill script documented in BACKFILL_INSTRUCTIONS.md with safety features (preview mode, idempotency, transaction safety).
*   **Client Portal Auto-Redirect to Projects (Nov 18, 2024):** Fixed client portal routing to always auto-redirect clients to their most recent project instead of showing questionnaires/estimates overview page. Backend `/api/client-portal` endpoint now sorts projects by `createdAt DESC` (newest first). Frontend client-portal.tsx page auto-redirects to first project when data loads, regardless of login method. Overview page only displays when client has zero projects. Ensures clients immediately see their active project upon login.
*   **Logo Upload Cloudinary Migration (Nov 18, 2024):** Changed photographer logo upload endpoint from local filesystem storage to Cloudinary CDN. Modified `/api/upload/logo` to use memoryStorage with uploadToCloudinary() function, ensuring logos persist across deployments and are accessible via permanent Cloudinary URLs.
